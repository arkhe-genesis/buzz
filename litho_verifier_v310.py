...
def run(path: Path, equipment_file: Optional[Path] = None, tolerance_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    for idx, claim in enumerate(claims):
        # Tenta encontrar o equipamento na claim atual
        detected_equip = identify_equipment(claim, equipment_db)
        if detected_equip:
            current_equipment = detected_equip
        ...
        # Track which parameters were cross-validated
        cross_validated_params = {cr.param_name for cr in cross_results}

        # Log UNVERIFIABLE for any parameters extracted but not known for this equipment
        for p, v, u in params:
            # Check if this extracted parameter was cross-validated
            # Extract names to match logic in cross_validate_parameters
            matched_name = None
            for prof_p in profile.parameters.keys():
                if prof_p == p or prof_p.replace('_', ' ') == p or prof_p == p.replace('_', ' '):
                    matched_name = prof_p
                    break
            ...
            if matched_name is None:
                validation_results.append({
                    ...
                    "extracted_unit": u,
                    "status": "UNVERIFIABLE",
                    "detail": "Parâmetro desconhecido",
                    "rationale": f"O parâmetro '{p}' não existe no perfil do equipamento '{current_equipment}'."
                })
...
class TestLithoVerifier(unittest.TestCase):
    ...
    def test_contextual_equipment_tracking(self):
        ...
        for idx, claim in enumerate(claims):
            ...
            if current_equipment:
                profile = EquipmentProfile.from_dict(current_equipment, equipment_db[current_equipment])
                cross_results = cross_validate_parameters(params, profile, {})
                for cr in cross_results:
                     validation_results.append({
                        "equipment": current_equipment,
                        "param_name": cr.param_name,
        ...
        self.assertEqual(validation_results[0]["equipment"], "Quantum X Shape")
        self.assertEqual(validation_results[0]["param_name"], "feature_size_xy")
        self.assertEqual(validation_results[0]["status"], "CONFIRMED")
        self.assertEqual(validation_results[1]["equipment"], "Quantum X Shape")
        self.assertEqual(validation_results[1]["param_name"], "feature_size_z")
        self.assertEqual(validation_results[1]["status"], "CONFIRMED")
