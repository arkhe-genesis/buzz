...
#[tokio::main]
async fn main() {
    println!("=== Timechain (Arkhe) — Simulação com 100 Nós ===");
    ...
    for step in 0..total_steps {
        ...
        if handover {
            ...
            let (u, s, vt) = f.omega_x.svd(true, true).unwrap();
            let u = u.unwrap();
            let vt = vt.unwrap();
            let shadow = Shadow::from_svd(&u, &s, &vt, 10);
            let echo = EchoSignal::new(shadow.strength(), h, step as u64);
            channel.emit(echo);
