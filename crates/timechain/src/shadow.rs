...
impl Shadow {
    pub fn from_svd(u: &Array2<f64>, s: &Array1<f64>, vt: &Array2<f64>, cut_rank: usize) -> Self {
        ...
        Self {
            ...
            energy_ratio: if total_energy > 0.0 { tail_energy / total_energy } else { 0.0 },
            cut_rank: k, total_rank,
        }
    }
    ...
    pub fn reconstruct(&self) -> Array2<f64> {
        ...
        for i in 0..m { diag[(i, i)] = self.tail_singular[i]; }
        self.tail_u.dot(&diag).dot(&self.tail_vt)
    }

    pub fn strength(&self) -> f64 { self.energy_ratio.sqrt() }
}

pub struct ShadowHealer { pub integration_rate: f64 }
impl ShadowHealer {
    pub fn new(integration_rate: f64) -> Self { Self { integration_rate } }
