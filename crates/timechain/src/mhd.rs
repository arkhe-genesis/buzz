...
pub struct PlasmaConfig {
    pub nu_base: f64, pub mu_phi: f64, pub rho_phi: f64,
    pub length: f64, pub nx: usize, pub ny: usize,
}
impl PlasmaConfig {
    pub fn new(nx: usize, ny: usize, length: f64, nu: f64) -> Self {
        Self { nu_base: nu, mu_phi: 1.0, rho_phi: 1.0, length, nx, ny }
    }
    pub fn dx(&self) -> f64 { self.length / (self.nx.saturating_sub(1).max(1)) as f64 }
    pub fn dy(&self) -> f64 { self.length / (self.ny.saturating_sub(1).max(1)) as f64 }
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct EvoField {
    pub omega_x: Array2<f64>, pub omega_y: Array2<f64>, pub omega_z: Array2<f64>,
...
impl EvoField {
    pub fn new(config: PlasmaConfig) -> Self {
        ...
        Self { omega_x: Array2::zeros(shape), omega_y: Array2::zeros(shape), omega_z: Array2::zeros(shape), config, nu_eff: config.nu_base }
    }
    ...
    pub fn random_harris(config: PlasmaConfig) -> Self {
        ...
        let lx = config.length; let ly = config.length;
        for i in 0..nx {
            ...
            for j in 0..ny {
                ...
                field.omega_y[(i, j)] = 0.0 + rng.gen::<f64>() * 0.01;
    ...
    pub fn current_density(&self) -> (Array2<f64>, Array2<f64>, Array2<f64>) {
        ...
        let dx = self.config.dx(); let dy = self.config.dy();
        ...
        for i in 1..nx-1 { for j in 1..ny-1 {
            ...
            jx[(i, j)] = doz_dy; jy[(i, j)] = -doz_dx; jz[(i, j)] = doy_dx - dox_dy;
        }}
        (jx, jy, jz)
    }
    ...
    pub fn compute_vector_potential(&self) -> (Array2<f64>, Array2<f64>, Array2<f64>) {
        ...
        for _ in 0..100 {
            ...
            for i in 1..nx-1 { for j in 1..ny-1 {
                ...
                ax[(i, j)] = 0.25 * (ax_old[(i+1, j)] + ax_old[(i-1, j)] + ax_old[(i, j+1)] + ax_old[(i, j-1)] + src_x);
                ay[(i, j)] = 0.25 * (ay_old[(i+1, j)] + ay_old[(i-1, j)] + ay_old[(i, j+1)] + ay_old[(i, j-1)] + src_y);
                az[(i, j)] = 0.25 * (az_old[(i+1, j)] + az_old[(i-1, j)] + az_old[(i, j+1)] + az_old[(i, j-1)] + src_z);
            }}
        }
        (ax, ay, az)
    }
    ...
    pub fn helicity(&self) -> f64 {
        ...
        let mut h = 0.0;
        for i in 0..self.config.nx { for j in 0..self.config.ny {
            h += ax[(i,j)] * self.omega_x[(i,j)] + ay[(i,j)] * self.omega_y[(i,j)] + az[(i,j)] * self.omega_z[(i,j)];
        }}
    ...
    pub fn energy(&self) -> f64 {
        let mut e = 0.0;
        for i in 0..self.config.nx { for j in 0..self.config.ny {
            e += self.omega_x[(i,j)].powi(2) + self.omega_y[(i,j)].powi(2) + self.omega_z[(i,j)].powi(2);
        }}
    ...
    pub fn advance(&mut self, dt: f64, ux: &Array2<f64>, uy: &Array2<f64>) {
        ...
        for i in 1..nx-1 { for j in 1..ny-1 {
            let lap_x = (self.omega_x[(i+1,j)] - 2.0*self.omega_x[(i,j)] + self.omega_x[(i-1,j)])/(dx*dx)
                      + (self.omega_x[(i,j+1)] - 2.0*self.omega_x[(i,j)] + self.omega_x[(i,j-1)])/(dy*dy);
            let lap_y = (self.omega_y[(i+1,j)] - 2.0*self.omega_y[(i,j)] + self.omega_y[(i-1,j)])/(dx*dx)
                      + (self.omega_y[(i,j+1)] - 2.0*self.omega_y[(i,j)] + self.omega_y[(i,j-1)])/(dy*dy);
            let lap_z = (self.omega_z[(i+1,j)] - 2.0*self.omega_z[(i,j)] + self.omega_z[(i-1,j)])/(dx*dx)
                      + (self.omega_z[(i,j+1)] - 2.0*self.omega_z[(i,j)] + self.omega_z[(i,j-1)])/(dy*dy);
            diff_x[(i,j)] = lap_x; diff_y[(i,j)] = lap_y; diff_z[(i,j)] = lap_z;
        }}
...
pub struct ReconnectionDetector {
    pub prev_helicity: f64, pub threshold: f64, pub handover_count: u32,
}
impl ReconnectionDetector {
    pub fn new(threshold: f64) -> Self { Self { prev_helicity: 0.0, threshold, handover_count: 0 } }
    pub fn detect(&mut self, field: &EvoField) -> bool {
        let h = field.helicity();
        let delta = (h - self.prev_helicity).abs();
        self.prev_helicity = h;
        if delta > self.threshold { self.handover_count += 1; true } else { false }
    }
}
